...
export default function RoomsPage() {
  ...
  const [topicMode, setTopicMode] = useState<TopicMode>("fixed");
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      const { data: room, error } = await supabase
        ...
        .insert({
          ...
          // In "random" mode there's no single fixed topic — a fresh one is
          // generated by the host's client each round instead (see
          // startRound() in the room page). The subject stands in here as a
          // non-empty placeholder since the column is required; it's never
          // actually shown or graded against once topic_mode is "random".
          topic: topicMode === "random" ? subject.trim() || t.rooms.subjectPlaceholder : topic.trim() || t.rooms.topicPlaceholder,
          level,
