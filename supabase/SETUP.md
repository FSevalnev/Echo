...
## 8. Включить Комнаты (Study Rooms)

Если ты уже настроил всё выше, для фичи "Комнаты" нужен ещё один шаг:

1. Заново запусти весь `supabase/schema.sql` в SQL Editor (он добавляет 4 новые таблицы: `rooms`, `room_participants`, `room_rounds`, `room_answers` — безопасно перезапускать).
2. В Supabase зайди в **Database → Replication** и включи Realtime для этих 4 таблиц (переключатель напротив каждой). Без этого шага живой рейтинг не будет обновляться сам — придётся вручную обновлять страницу.
